...
namespace DACS_TimeManagement.Services
{
    public class GeminiService : IGeminiService
    {
        ...
        public GeminiService(HttpClient httpClient, IConfiguration configuration, ILogger<GeminiService> logger)
        {
            ...
            _model = _config["Gemini:ModelName"] ?? "gemini-1.5-flash";
        ...
        {
            var modelsToTry = new System.Collections.Generic.List<string> { _model };
            ...
            foreach (var modelName in modelsToTry)
            {
                ...
                {
                    _logger.LogError(ex, "HTTP request failed for model {Model}", modelName);
                    lastError = $"HTTP request failed for {modelName}: {ex.Message}";
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!doc.RootElement.TryGetProperty("candidates", out var candidates) || candidates.GetArrayLength() == 0)
                    {
                        _logger.LogWarning("No candidates in Gemini response for model {Model}. Body: {Body}", modelName, responseBody);
                        lastError = "AI không đưa ra phản hồi nào (có thể do nội dung bị chặn hoặc prompt không hợp lệ).";
                        continue; // Try next model
                    }
                    ...
                    if (firstCandidate.TryGetProperty("finishReason", out var finishReasonProp))
                    {
                        ...
                        {
                            _logger.LogWarning("Gemini finish reason for model {Model}: {FinishReason}", modelName, finishReason);
                            lastError = finishReason switch
                            {
                    ...
                    _logger.LogInformation("Gemini response missing text structure for model {Model}. Body: {Body}", modelName, responseBody);
                    lastError = "AI không thể tạo phản hồi ở định dạng mong đợi.";
                    continue; // Try next model
                }

                _logger.LogWarning("Gemini API error for model {Model}. Status: {Status}, Body: {Body}", modelName, response.StatusCode, responseBody);
                lastError = HandleApiError(response.StatusCode, responseBody);
